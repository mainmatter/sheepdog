...
If you don't know anything about what a Vite plugin even is and want to know more here's a
...
<details>
    ...
    Here's an example of a simple Vite plugin very similar to one in <BlankLink href="https://vitejs.dev/guide/api-plugin" body="their documentation" />
...
<Aside type="caution">
	Don't use this anywhere. It's just a silly example and you should probably
	be a bit more nuanced than `replaceAll` if you want to remove logs from your
	code.
</Aside>
...
The reason why we are writing this explainer is because we firmly believe that if you allow your code
to be changed by some script you should fully understand how it changes it and what the final result
will look like.
...
To fix these problems, we let you write the simple and familiar async code, and we provide a Vite
...
because we specifically target only the async functions that are arguments of our own
