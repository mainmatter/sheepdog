...
While defining a task if the function that you pass in has some arguments those will be required by
the `perform` function (and it will be strongly typed too).
